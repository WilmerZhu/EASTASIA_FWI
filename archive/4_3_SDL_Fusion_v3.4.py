...
class Phase0Config:
    ...
    validation_depths: List[float] = field(
        default_factory=lambda: [10, 30, 60, 100]
    )
...
class Phase1Config:
    ...
    ⭐ v3.4 参数与论文对比:
    - 物理窗口: 当前 6°×6° vs 论文 ~6×0.03°≈0.18° (大33倍)
    - 步长: 当前 1° vs 论文 stride=1格点≈0.03° (大33倍)
    - D₂正则化: 0 (纯最小二乘，与论文一致) ✅
    ...
    # === 物理窗口配置 ⭐ 测试模型对 USTClitho(0.5°)→CSES(0.25°) ===
    physical_window_size: float = 3.0    # 3°×3° 物理窗口 (M₁: 6×6 格点, M₂: 12×12 格点)
    physical_stride: float = 0.5         # 0.5° 步长 (=1 格点 @0.5°，83%重叠)
    # 说明: CSES 范围小 (9.5°×14°)，选用 3° 窗口 + 0.5° 步长以获得足够 patches
    ...
    depth_match_tolerance_km: float = 0.5


@dataclass
class Phase2Config:
    """Phase 2: 3D Patch融合配置"""
    
    # 启用开关
    enabled: bool = False  # 默认不启用
    
    # === 3D物理窗口配置 ===
    physical_window_lat: float = 6.0     # 6°
    physical_window_lon: float = 6.0     # 6°
    physical_window_depth: float = 200.0  # 200km
    
    physical_stride_lat: float = 3.0
    physical_stride_lon: float = 3.0
    physical_stride_depth: float = 100.0
    ...
    # === 深度范围 ===
    depth_range: Tuple[float, float] = (0, 600)  # 专注上地幔
...
class SDLConfigV31:
    ...
    def __init__(self):
        # ============ 模型信息 ============
        # ⭐ 测试模型对: 大范围粗分辨率 USTClitho2.0 (M₁) → 小范围高分辨率 CSES_VM1.0 (M₂)
        # USTClitho2.0: 0.5°×0.5°, 72-136°E, 18-54°N, 0-150km (12层不规则深度)
        # CSES_VM1.0:   0.25°×0.25°, 97.5-107°E, 21-35°N, -6-120km (47层) ⊂ USTClitho
        self.model_info: Dict[str, ModelResolutionInfo] = {
            '2022_USTClitho2.0': ModelResolutionInfo(
                name='USTClitho2.0',
                full_name='2022_USTClitho2.0',
                lat_resolution=0.5,
                lon_resolution=0.5,
                depth_resolution=13.6,
                color='#3498DB',
                marker='o',
                role='base',
                filename='2022_USTClitho2.0_original.nc',
                short_name='ust'
            ),
            '2024_CSES_VM1.0': ModelResolutionInfo(
                name='CSES_VM1.0',
                full_name='2024_CSES_VM1.0',
                lat_resolution=0.25,
                lon_resolution=0.25,
                depth_resolution=2.74,
                color='#E74C3C',
                marker='s',
                role='highres',
                filename='2024_CSES_VM1.0_original.nc',
                short_name='cses'
            )
        }
        
        # ============ 核心参数 ============
        self.base_model_key: str = '2022_USTClitho2.0'
        self.highres_model_keys: List[str] = ['2024_CSES_VM1.0']
        self.default_target_model: str = '2024_CSES_VM1.0'
...
class ModelLoader:
    ...
    def __init__(self, logger: logging.Logger):
        ...
        # 模型简称映射
        self.short_name_map = {
            '2022_USTClitho2.0': 'ust',
            '2024_CSES_VM1.0': 'cses'
        }
    ...
    def _extract_depth_slice(self, data: np.ndarray, dims: List[str],
    ...
    def extract_paired_patches_direct(self,
        ...
        for lat_c in lat_centers:
            for lon_c in lon_centers:
                ...
                base_upsampled = zoom(base_patch, 
                                      highres_patch_size / base_patch_size,
                                      order=3)
...
class TwoDTransformer:
    ...
    def run(self,
            ...
            param: str,
            depth_km: float,
            target_model: str = 'cses') -> TransformResult:
        """
        在指定深度执行2D变换
        ...
        model_key_map = {
            ...
            '2024_CSES_VM1.0': '2024_CSES_VM1.0',
            '2022_USTClitho2.0': '2022_USTClitho2.0'
        }
...
class FullSliceTransformer:
    ...
    def transform_full_slice(self,
        ...
        for lat_c in lat_centers:
            for lon_c in lon_centers:
                ...
                base_patch_lowres = zoom(base_patch_highres, scale_factor, order=1)
    ...
    def transform_full_slice_direct(self,
        ...
        base_depth_idx = np.argmin(np.abs(base_depth - depth_km))
        highres_depth_idx = np.argmin(np.abs(highres_depth - depth_km))
    ...
    def _extract_depth_slice_direct(self, data: np.ndarray, dims: List[str],
                                    lat_name: str, lon_name: str,
                                    depth_idx: int) -> np.ndarray:
        """根据维度顺序提取深度切片"""
        # 找到各维度索引
...
class SDLVisualizerV31:
    ...
    def _create_geo_axes(self, fig, nrows: int = 1, ncols: int = 1, 
                         index: int = 1, figsize: Tuple[int, int] = None) -> plt.Axes:
        """
    ...
    def _plot_U02_depth_coverage_direct(self, model_loader: 'ModelLoader', 
        ...
        models_info = [
            ('2022_USTClitho2.0', 'USTClitho2.0', '#3498DB'),
            ('2024_CSES_VM1.0', 'CSES_VM1.0', '#E74C3C')
        ]
    ...
    def _plot_T04b_paired_atoms(self, cd: CoupledDictionaryResult, 
        ...
        for k in range(n_show):
            ...
            atom1_up = zoom(atom1, side2 / side1, order=3)
    ...
    def _plot_F01_full_slice_comparison(self, 
        ...
        for depth_km, result in results.items():
            ...
            im2 = ax2.pcolormesh(lon, lat, result.original_slice,
                                 cmap=self.config.velocity_cmap,
                                 vmin=vmin, vmax=vmax, transform=ccrs.PlateCarree())
            ax2.set_title(f'(b) USTClitho2.0 (Original)', fontsize=12)
            ax2.set_extent([lon.min(), lon.max(), lat.min(), lat.max()], crs=ccrs.PlateCarree())
            self._add_coastlines(ax2)
            plt.colorbar(im2, ax=ax2, label=f'{param.upper()} (km/s)', shrink=0.8)
...
class HyperparameterSearcher:
    ...
    def search(self,
               ...
               param: str = 'vs',
               target_model: str = 'cses') -> Optional[GridSearchResult]:
        """
        执行 K × λ 联合网格搜索（论文方法）
    ...
    def _run_single_test(self, config_name: str, depths: List[float],
        ...
        try:
            ...
            return GridSearchResult(
                ...
                transform_error=np.mean(all_errors),
                baseline_error=np.mean(all_baseline_errors),
                relative_improvement=np.mean(all_improvements),
...
class SDLFusionV33:
    ...
    def run_phase0(self, param: str) -> Dict[str, Dict[float, ValidationResult]]:
        ...
        # 验证各模型 - 使用原始分辨率
        models_to_validate = [
            ('USTClitho2.0', 'ust', '2022_USTClitho2.0'),
            ('CSES_VM1.0', 'cses', '2024_CSES_VM1.0')
        ]
    ...
    def run_phase1(self, param: str, target_model: str,
        ...
        target_model_key_map = {'cses': '2024_CSES_VM1.0', 'ust': '2022_USTClitho2.0'}
        highres_model_key = target_model_key_map.get(target_model, target_model)
        ...
        self.logger.info(
            f"    USTClitho2.0 深度: {len(base_depths)} 层 "
    ...
    def run_phase1_5(self, param: str, target_model: str,
        ...
        for depth_km in tqdm(transform_depths, desc="    Phase 1.5 Transform", 
            ...
            try:
                ...
                base_model_key = self.config.base_model_key  # '2022_USTClitho2.0'
