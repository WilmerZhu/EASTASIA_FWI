...
class StationNetworkPlotter:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        ...
        # 自动提取模块号（从文件名如 5_3_All_Stations.py 提取 5-3）
        self.module_prefix = extract_module_prefix(Path(__file__), default="5-3")
        
        # 生成颜色方案（40种颜色：tab20的20种 + tab20b的20种）
        # 使用新的 matplotlib.colormaps API（避免弃用警告）
        tab20_cmap = matplotlib.colormaps['tab20']
        tab20b_cmap = matplotlib.colormaps['tab20b']
...
"""
    ...
    def plot_combined_station_network_map(self, 
        ...
        if save_file is None:
            save_file = generate_filename(self.module_prefix, "combined_station_network_distribution")
    ...
    def plot_station_statistics(self, 
        ...
        # 如果没有指定文件名，自动生成
        if save_file is None:
            save_file = generate_filename(self.module_prefix, "station_comprehensive_statistics")
    ...
    def plot_station_deployment_timeline(self, 
        ...
        # 如果没有指定文件名，自动生成
        if save_file is None:
            save_file = generate_filename(self.module_prefix, "station_deployment_timeline")
