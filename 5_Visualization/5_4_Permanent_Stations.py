"""
EASTASIA-FWI 台站网络分析和可视化系统（完善图例和颜色版）
主要功能：
1. 所有台站分布图
2. 永久台站分布图（>5年且EndDate为空）- 完整图例
3. 永久台网时间线分析图 - 所有台网，颜色一致
4. 密集台网筛选（0.5度范围内>5个台站的台网）
5. 排除合成台站（SY台网等）
6. 正确的台站去重处理
"""
    ...
    def _preprocess_station_data(self):
        ...
        try:
            ...
            if excluded_count > 0:
                ...
                self.logger.info(f"🚫 排除合成/测试台网: {list(excluded_networks_found)} ({excluded_count} 个台站)")
...
def main():
    """主函数 - 完善图例和颜色版台站网络分析系统"""
    print("🎯 EASTASIA-FWI 台站网络分析系统（完善图例和颜色版）")
    print("   主要功能：")
    print("   1. 所有台站分布图（已去重）")
    print("   2. 永久台站分布图（>5年且活跃，排除密集台网）- 完整图例")
    print("   3. 年度永久台站部署统计图（独立绑制）")
    print("   4. 永久台网时间线图（3:4比例，独立绑制）")
    print("   5. 台站分布图 + 震源球叠加")
    print("   6. 自动排除合成台站（SY等）和密集台网")
    print("   7. 严格的东亚研究区域限制")
    print("   8. 基于StationID的智能去重")
    print("=" * 70)
    ...
    try:
        ...
        focal_result = analyzer.plot_permanent_stations_with_focal_mechanisms(
            min_magnitude=7.0,  # 可调整最小震级
            max_events=50       # 可调整最大事件数
        )
        if focal_result:
        ...
        if results:
            ...
            descriptions = {
                'all_stations_distribution': '所有台站分布图（已去重）',
                'permanent_stations_distribution': '永久台站分布图（筛选后）',
                'annual_deployment': '年度永久台站部署统计图',
                'permanent_networks_timeline': '永久台网时间线图（3:4比例）',
                'stations_with_focal_mechanisms': '台站+震源球叠加图'
            }
            ...
            if analyzer.all_stations is not None:
                ...
                print(f"   所有网络: {analyzer.all_stations['Network'].nunique()} 个")
            ...
            if analyzer.filtered_permanent_stations is not None:
                ...
                print(f"   筛选后网络: {analyzer.filtered_permanent_stations['Network'].nunique()} 个")
            
            print(f"\n📁 所有文件保存在: {analyzer.output_dir}")
