...
功能描述:
----------
基于PyGMT绘制东亚地区高质量基础地形图和地球物理要素图。
提供完整的地质构造背景，包括高分辨率地形、海底年龄、火山分布、断层、板块边界和俯冲带等深线，
为地震学数据可视化提供专业的地图底图。

核心功能:
----------
1. ✅ 高分辨率地形绘制
   - 使用GMT全球地形数据 (@earth_relief_01m)
   - 支持多种地形色标 (terra, dem等)
   - 可配置地形高程范围
...
----------
```python
from 5_Visualization.5_1_Basemap import EastAsiaBasemap
...
配置说明:
----------
通过 BasemapConfig 类配置绘制参数:
- pygmt: PyGMT地图框架配置 (字体、网格、标注等)
- projection: 地图投影配置 (默认墨卡托投影M15c)
- data_sources: 数据源配置 (地形、海底年龄数据)
- colors: 色彩配置 (地形、年龄、俯冲带色标)
- layers: 图层显示配置 (各要素开关)
- output: 输出配置 (DPI、格式、裁剪)

输出文件:
----------
- 5-1_comprehensive_basemap.jpg/pdf: 完整版地质构造图
- 5-1_simple_basemap.jpg/pdf: 简化版基础地形图
- 所有图片保存在 figures/ 目录

科学原理:
----------
- 地形数据: GMT全球地形数据提供高分辨率地形背景，帮助理解区域构造背景
- 海底年龄: 反映板块扩张历史，有助于理解区域构造演化
- 火山分布: 与板块边界和俯冲带密切相关，指示构造活动性
- 板块边界: 显示主要构造边界，是理解地震分布的关键
- 俯冲带等深线: Slab2数据提供俯冲板片几何形态，对深源地震研究至关重要
- 地图投影: 墨卡托投影适合中纬度地区，保持方向和形状的良好平衡
