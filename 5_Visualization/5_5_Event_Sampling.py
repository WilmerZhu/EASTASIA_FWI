"""
东亚地区事件采样器
基于筛选后的永久台站进行GCMT事件采样并生成SPECFEM3D globe文件
配置参数已集成在模块内部
核心原则：永久台站覆盖好的事件 + 随机采样
...
def main():
    """主函数 - 事件采样系统"""
    print("🎯 EASTASIA-FWI 事件采样系统")
    print("=" * 70)
    print("   主要功能：")
    print("   1. 基于筛选后的永久台站进行GCMT事件采样")
    print("   2. 随机加权采样，确保台站覆盖质量")
    print("   3. 生成SPECFEM3D Globe所需的STATIONS和CMTSOLUTION文件")
    print("   4. 生成PAR格式事件目录文件（用于波形下载）")
    print("   5. 创建2D地图、3D分布图和统计图表（英文标签）")
    print("   6. 创建交互式3D可视化（需要Plotly）")
    print("   7. 提供详细的采样质量分析")
    print("=" * 70)
    
    # 检查Plotly可用性
    if PLOTLY_AVAILABLE:
        print("✅ Plotly已安装，支持交互式3D可视化")
    else:
        print("⚠️  Plotly未安装，将跳过交互式3D可视化")
    ...
    try:
        ...
        if results['selected_events_count'] > 0:
            ...
            # 显示采样统计
            stats = results['sampling_statistics']
            print(f"\n📈 采样统计:")
            print(f"   震级范围: {stats['magnitude_range'][0]:.2f} - {stats['magnitude_range'][1]:.2f}")
            print(f"   深度范围: {stats['depth_range'][0]:.1f} - {stats['depth_range'][1]:.1f} km")
            print(f"   平均台站覆盖: {stats['station_coverage']['mean_stations_per_event']:.1f} 个/事件")
            print(f"   平均覆盖评分: {stats['station_coverage']['mean_coverage_score']:.3f}")
            
            # 显示深度分布
            print(f"\n📏 深度分布:")
            for bin_name, count in stats['depth_distribution'].items():
                print(f"   {bin_name.title()}: {count} 个事件")
            
            # 显示SPECFEM文件
            if 'specfem_files' in results and results['specfem_files']:
                print(f"\n📄 生成的文件:")
                specfem = results['specfem_files']
                if 'stations_file' in specfem:
                    print(f"   STATIONS文件: {Path(specfem['stations_file']).name}")
                if 'cmtsolution_files' in specfem:
                    print(f"   CMTSOLUTION文件: {specfem['cmtsolution_files']} 个")
                if 'par_catalog' in specfem:
                    print(f"   PAR事件目录: {Path(specfem['par_catalog']).name}")
                if 'event_list' in specfem:
                    print(f"   事件列表文件: {Path(specfem['event_list']).name}")
            
            # 显示可视化文件
            if 'visualizations' in results:
                print(f"\n🎨 可视化图表:")
                viz_descriptions = {
                    'station_event_map': '2D台站-事件分布地图',
                    '3d_distribution': '3D事件分布图（静态，无海岸线）',
                    'interactive_3d': '交互式3D分布图（HTML，无海岸线）',
                    'sampling_statistics': '采样统计图表'
                }
                
                for key, file_path in results['visualizations'].items():
                    desc = viz_descriptions.get(key, key)
                    print(f"   {desc}: {Path(file_path).name}")
                    
                    # 特别提示交互式3D图的使用方法
                    if key == 'interactive_3d':
                        print(f"      💡 在浏览器中打开此HTML文件可进行交互操作")
            
            # 显示报告文件
            if 'report_file' in results and results['report_file']:
                print(f"\n📋 分析报告: {Path(results['report_file']).name}")
            
            print(f"\n📁 所有文件保存在: {sampler.output_dir}")
            print(f"📁 PAR和事件文件保存在: {sampler.base_config.dirs['events']}")
